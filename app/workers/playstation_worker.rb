...
SOURCES = {
  ...
  fust: {
    ...
    match: "ir aufgrund der sehr hohen Nachfrage nicht"
  },
  gamestop: {
  ...
  mediamarkt: {
    ...
    match: "Produkt momentan nicht"
  },
}

class PlaystationWorker
  ...
  def perform  
    result = SOURCES.map do |key, value|
      ...
      {source: key, available: available}
    end

    ActionCable.server.broadcast("ps5stock", result)
  end 
